...
for p in params.keys():
    if p not in ['email', 'year']:
        ...
        db.add_to_enrollment_form(p, session_id, status)
session_dict = db.get_data(mydb,session_id)
subjects = db.get_subjects()

if os.environ['REQUEST_METHOD'] == 'POST':
    if params.getvalue("logout") == "Logout":
        db.logout()
    ...
    elif params.getvalue("student_list") == "Student list":
        if(db.check_role(session_id)):
            print("Location: popis.py?email=" + email + "\r\n\r\n")
        else:
            message = "Admin access rights required"
